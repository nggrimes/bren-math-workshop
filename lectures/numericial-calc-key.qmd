...
For example, I never showed in class how to solve trigonometric derivaties. Use `D()` to find the derivative of $sin(y^2)$
...
```{r}
df_crazy<-D(3*x^2+2*x^3+sin(x^2)+cos(x)~x)

#plot the original function and df_crazy from -5 to 5
x=seq(-5,5,by=.01)
f_crazy=3*x^2+2*x^3+sin(x^2)+cos(x)
df_crazy_y=df_crazy(x)

ggplot()+
  geom_line(aes(x=x,y=f_crazy,color="Original Function"))+
  geom_line(aes(x=x,y=df_crazy_y,color="Derivative"))+
  theme_classic()+
  labs(color="Function")+
...
For definte integrals I actually prefer the base R `integrate` function. Same idea, but now we need to define our own functions beforehand.
...
The optimal value was found at `r `xstar`. By taking the second derivative and evaluating it at `r xstar`, we can see that it is a minimum by the 2nd order conditions ($f''(x^*)>0$).
